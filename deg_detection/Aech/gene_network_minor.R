...
mpha_exp = log2(abundance[,seq(1,12,3)]/abundance[,seq(3,12,3)])#assay(vsd)
...
caste_bias = data.frame("Caste.expression" = gene_age$caste[match(tmp_caste, gene_age$geneID)],
                        row.names = tmp_caste)
caste_bias = data.frame("Caste.expression" = res_gs$log2FoldChange[match(tmp_caste, rownames(res_gs))],
                        row.names = tmp_caste)
...
caste_ac = data.frame(ac = apply(abs(caste_net),1,mean),
                      ...
                      log2 = res_gs$log2FoldChange[match(caste_gene, rownames(res_gs))])
head(caste_ac)
boxplot(ac ~ age, data = caste_ac)
write.table(caste_ac,file = 'Aech_caste_ac.txt',sep = '\t',quote = F)
