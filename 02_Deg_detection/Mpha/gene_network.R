...
#table(gene_age$caste, gene_age$age)/rep(c(array(table(gene_age$age))),each = 3)
#t.test(gene_age$ac[gene_age$caste == 'gyne'],gene_age$ac[gene_age$caste == 'non-bias'])
...
levels(gene_age$age) = c('Neoptera','Endopterygota','Hymenoptera','Aprocrita','Aculeata','Formicidae','Myrmicinae','Lineage specific')
levels(gene_age$caste) = c("Gyne bias","Worker bias","NDE")
n = 500
tmp_caste = as.character(sample(gene_age[which(gene_age$caste %in% c('Gyne bias','Worker bias',"NDE")),'geneID'], n))
tmp = tmp_caste
...
ann_colors = list(
  Evolutionary.origin = c(Neoptera =tmp_col[1],Endopterygota = tmp_col[2],Hymenoptera=tmp_col[3],Aprocrita = tmp_col[4],Aculeata = tmp_col[5],
                          Formicidae = tmp_col[6],Myrmicinae = tmp_col[7],`Lineage specific` = tmp_col[8]),
...
pheatmap((mpha_net[tmp_caste,tmp_caste]), cluster_rows=T, show_rownames=F,show_colnames = F, cluster_cols=T,annotation_col=age_bias,annotation_row = caste_bias,
         annotation_colors = ann_colors,main = "M.pharaonis")
#####

connection= rowSums(abs(mpha_net[tmp_caste,tmp_caste]))/n
colors <- colorRampPalette( (brewer.pal(9, "Blues")) )(255)
pheatmap(t(connection), cluster_rows=F, show_rownames=F,show_colnames = F, cluster_cols=T,annotation_colors = ann_colors,annotation_col=age_bias,
         clustering_distance_cols = dist(abs(mpha_net[tmp_caste,tmp_caste])),color = colors,cellheight = 100)

pheatmap(abs(mpha_net[tmp,tmp]), cluster_rows=T, show_rownames=F,show_colnames = F, cluster_cols=T,annotation_col=age_bias,
...
write.table(caste_ac,file = 'Mpha_caste_ac.txt',sep = '\t',quote = F)
...
caste_ac_tmp=  (caste_ac[caste_ac$caste != 'NDE',])
boxplot(ac ~ age, data = caste_ac_tmp)

table(caste_ac_tmp$age)
summary(lm(ac ~ age, data = caste_ac_tmp))
