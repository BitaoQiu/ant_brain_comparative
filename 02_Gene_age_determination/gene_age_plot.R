...
#Import the output from gene_age_summary.R (A.echinatior first)
...
gene_age_aech_wide[,c(8:10)] = binom.confint(gene_age_aech_wide$Gyne,rep(sum(gene_age_aech_wide$Gyne),8),conf.level = 0.9,methods = 'exact')[,c(4:6)]/
  rowSums(gene_age_aech_wide[,c(2:6)])*sum(gene_age_aech_wide[,c(2:6)]) #Using binomial distribution to calculate the confidence interval.
