...
pathway_info = data.frame(row.names = names(go.bp),
                          N_origin = rep(NA,length(names(go.bp))), 
                          N_gene = rep(NA,length(names(go.bp))), 
                          bias_value = rep(NA,length(names(go.bp))),
                          sd = rep(NA,length(names(go.bp))),
                          cut_off = rep(NA,length(names(go.bp))),
                          cut_off_N = rep(NA,length(names(go.bp))),
                          abs_bias_value = rep(NA,length(names(go.bp))),
                          pvalue = rep(NA,length(names(go.bp))),
                          abs_pvalue = rep(NA,length(names(go.bp))))
...
for(i in names(go.bp)){
  ...
  if(!is.null(dim(tmp_data))){
    if (dim(tmp_data)[1] > 1){
      ...
      pathway_info[i,'N_origin'] = length(go.bp[i][[1]])
...
pathway_info[order(pathway_info$abs_bias_value,decreasing = T)[1:20],]
pathway_info[order(pathway_info$cut_off,decreasing = T)[1:20],]
pathway_info[grep('beha',rownames(pathway_info)),]
out = pathway_info[order(pathway_info$bias_value,decreasing = T)[1:30],]
